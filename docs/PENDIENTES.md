...
- [ ] **Campaña `windsor_nuevo_ciclo` (2026-09-04) — enviado a revisión en Meta, falta confirmar aprobación:** el texto es genérico a propósito (aplica a cualquier oferta educativa), así que reactiva a **2,119 leads pendientes de TODOS los programas** (no solo licenciaturas+idiomas como se planteó al inicio). Excluye: sin whatsapp, stage `inscrito`/`perdido`/`cerrado` (`cerrado` es nombre legacy de `inscrito`, ver `LEGACY_STAGE_MAP` en `app/crm.jsx` — bug detectado y corregido en el script, antes se colaban 22 leads ya inscritos), y RR.PP./Mercadotecnia (ya contactados con `lic_mkt` el 2026-09-03). Texto: "Hola {{1}} 👋 Se acerca el nuevo ciclo escolar en Instituto Windsor y tenemos promociones vigentes para ti. Por favor, selecciona una opción." — botones (Meta sí permite más de 3 Quick Reply tipo Custom, corrección de lo dicho antes): "Me quiero inscribir" / "Tengo dudas" / "Quizás más adelante" / "Ya no me interesa", categoría Marketing. Ya configurado en WhatsApp Manager por Harold (2026-09-04), pendiente confirmar aprobación de Meta. Al enviar, el script mueve al lead a stage `promocion_enviada`; al responder, el webhook auto-clasifica (interceptores en `app/api/whatsapp/webhook/route.ts`, buscar "windsor_nuevo_ciclo" y "seguimiento_general"): "Me quiero inscribir" → `inscripcion_pendiente`, "Tengo dudas" → `tercer_contacto`, "Quizás más adelante" → `archivado`, "Ya no me interesa" → `perdido`. **Ritmo final acordado (2026-09-04) — conservador, exponencial:** Día 1: 50, Día 2: 100, Día 3: 200, Día 4: 400, Día 5: 800, Día 6: resto (~569), dentro del tope de 2,000/día de la cuenta (confirmado en WhatsApp Manager). Script listo: `scripts/campana-ciclo-escolar.mjs --send --limit=50` una vez aprobado el template.
- **Bug encontrado y corregido (2026-09-04) — leads de "verano" recibían inscripción a My Best Summer ya cerrado:** al decir "quiero inscribirme"/"quiero apartar mi lugar", cualquier lead con curso de verano (Cursos de verano niños/adultos, Verano niños/adultos — cientos de leads en la campaña actual) recibía el proceso de inscripción activo a *My Best Summer 2026*, que según `reglasNegocio.ts` ya concluyó y el bot nunca debe ofrecer. Detectado vía la bandera 🚩 (lead Tania Itzel). Corregido: `INSCRIPCION_VERANO_NINOS_MSG`/`INSCRIPCION_VERANO_ADULTOS_MSG` en `app/api/whatsapp/webhook/route.ts` ahora avisan que esa edición ya cerró y redirigen explícitamente a la info del curso regular de idiomas (abierto todo el año), reusando `INFO_MSGS` + `buildCTA`. Un solo punto de arreglo cubre los 3 flujos que usaban este mensaje (interceptor de "me quiero inscribir", fase `inscripcion` para verano, y campaña `reactivacion_verano`).
- **Bug encontrado y corregido (2026-09-04) — fase `inscripcion` repetía el checklist ante cualquier pregunta:** en Bachillerato/Diplomado/Habilidades Psicoterapéutica/Idiomas, una vez en fase `inscripcion` el bot reenviaba el mensaje fijo completo de documentos sin importar qué preguntara el lead después (caso real: Naomi Yamileth Dimas Arcos preguntó "puedo checar los horarios" y recibió otra vez el checklist completo). Ya tenía el escape "si parece pregunta → cae a GPT" en fase `inscripcion_pendiente` (verano) pero faltaba en `inscripcion`. Corregido en `app/api/whatsapp/webhook/route.ts` (busca `esPreguntaInsc`), desplegado.

*Última actualización (2026-09-04): script y auto-clasificación por webhook de la campaña `windsor_nuevo_ciclo` listos; falta confirmar aprobación del template en Meta antes de enviar el primer lote.*
